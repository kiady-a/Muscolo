...
function getGroup($idUtilisateur) {
    $db = myPdo();
    $requete = $db->prepare("select * from groupe where idUtilisateur = :idUtilisateur");
    $requete->execute(array('idUtilisateur' => $idUtilisateur));
    return $requete;
}

function isNotEmptyGroup($idUtilisateur) {
    $db = myPdo();
    $requete = $db->prepare("select * from groupe where idUtilisateur = :idUtilisateur");
    $requete->execute(array('idUtilisateur' => $idUtilisateur));
...
function addGroup($name, $group, $idUser) {
    $db = myPdo();
    $request = $db->prepare("INSERT INTO groupe(participants, nom, idUtilisateur) VALUES (:group, :name, :idU);");
    $request->execute(array('group' => $group,
        'name' => $name,
        'idU' => $idUser));
    return $request;
}

function deleteGroup($idGroupe) {
  $db = myPdo();
  $request = $db->prepare("DELETE FROM `groupe` WHERE `groupe`.`idGroupe` = :idGroupe");
  $request->execute(array('idGroupe' => $idGroupe));
  return $request;
}
